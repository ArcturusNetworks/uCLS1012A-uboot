...
int spi_flash_cmd_erase_ops(struct spi_flash *flash, u32 offset, size_t len)
{
	...
	if (flash->flash_is_locked) {
		if (flash->flash_is_locked(flash, offset, len) > 0) {
			printf("offset + len: 0x%lx is protected and cannot be erased\n",
			       offset + len);
			return -EINVAL;
		}
	}
		...
		size_t len, const void *buf)
{
	...
	if (flash->flash_is_locked) {
		if (flash->flash_is_locked(flash, offset, len) > 0) {
			printf("offset + len: 0x%lx is protected and cannot be written\n",
			       offset + len);
			return -EINVAL;
		}
	}
...
/*
 * Check if a region of the flash is (partial) locked. See spansion_lock() for
 * more info.
 *
 * Returns 1 partial region is locked, 0 if entire is unlocked, and
 * negative on errors.
 */
int spansion_is_partial_locked(struct spi_flash *flash, u32 ofs, size_t len)
{
	u8 cmd[4];
	...
	stm_get_locked_range(flash, sr1nv, &lock_offs, &lock_len);

	if (lock_len)
		ret = (ofs + len >= lock_offs) ? 1 : 0;
	if (ret)
		printf("lock_offset: 0x%llx  lock_len: %#x\n",
				lock_offs, lock_len);
...
int spansion_is_locked(struct spi_flash *flash, u32 ofs, size_t len)
{
	...
	ret = spansion_is_locked_sr(flash, ofs, len, sr1nv) ||
	      spansion_is_partial_locked(flash, ofs, len);
